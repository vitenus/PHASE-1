...
def save_service_request(client, request_data: dict):
    return client.table('service_requests').insert(request_data).execute().data[0]


def save_proposal(client, proposal_data: dict):
    return client.table('service_proposals').insert(proposal_data).execute().data[0]
